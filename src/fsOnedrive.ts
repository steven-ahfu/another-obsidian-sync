...
export class FakeFsOnedrive extends FakeFs {
  readonly serviceType = "onedrive" as const;
  kind: "onedrive";
  onedriveConfig: OnedriveConfig;
  ...
  async _putUint8ArrayByRange(
    ...
    if (false /*VALID_REQURL*/) {
      ...
      const res = await fetch(theUrl, {
        method: "PUT",
        body: payload.subarray(rangeStart, rangeEnd) as Uint8Array<ArrayBuffer>,
        headers: {
  ...
  async walk(): Promise<Entity[]> {
    ...
    log.debug(`[fsOnedrive] walk: found ${unifiedContents.length} entities`);
  ...
  async writeFile(
    ...
    log.debug(`[fsOnedrive] writeFile: key=${key} size=${content.byteLength}`);
    await this._init();
  ...
  async readFile(key: string): Promise<ArrayBuffer> {
    log.debug(`[fsOnedrive] readFile: key=${key}`);
    await this._init();
    if (key.endsWith("/")) {
  ...
  async rm(key: string): Promise<void> {
    ...
    log.debug(`[fsOnedrive] rm: key=${key}`);
