...
export class FakeFsDropbox extends FakeFs {
  readonly serviceType = "dropbox" as const;
  kind: "dropbox";
  ...
  async _walk(partial: boolean): Promise<Entity[]> {
    ...
    log.debug(`[fsDropbox] walk: found ${unifiedContents.length} entities`);
  ...
  async writeFile(
    ...
    log.debug(`[fsDropbox] writeFile: key=${key} size=${content.byteLength}`);
    await this._init();
    const uploadFile = getDropboxPath(key, this.remoteBaseDir);
  ...
  async readFile(key: string): Promise<ArrayBuffer> {
    log.debug(`[fsDropbox] readFile: key=${key}`);
    await this._init();
    if (key.endsWith("/")) {
  ...
  async rm(key: string): Promise<void> {
    ...
    log.debug(`[fsDropbox] rm: key=${key}`);
