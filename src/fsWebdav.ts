...
import { VALID_REQURL } from "./baseTypes";
...
export class FakeFsWebdav extends FakeFs {
  readonly serviceType = "webdav" as const;
  kind: "webdav";
  ...
  async _checkPartialSupport() {
    const compliance = await (this.client as any).getDAVCompliance(
      `/${this.remoteBaseDir}/`
    );
  ...
  async walk(): Promise<Entity[]> {
    ...
    const result = contents
      ...
      .filter((x) => x.keyRaw !== "/");
    log.debug(`[fsWebdav] walk: found ${result.length} entities`);
    return result;
  }
  ...
  async writeFile(
    ...
    log.debug(`[fsWebdav] writeFile: key=${key} size=${content.byteLength}`);
    await this._init();
    const uploadFile = getWebdavPath(key, this.remoteBaseDir);
  ...
  async _writeFileFromRootApachePartial(
    ...
    for (let i = 0; i < chunkRanges.length; ++i) {
      const { start, end } = chunkRanges[i];
      await (this.client as any).partialUpdateFileContents(
        key,
        start,
        end,
        content.slice(start, end + 1)
  ...
  async readFile(key: string): Promise<ArrayBuffer> {
    ...
    log.debug(`[fsWebdav] readFile: key=${key}`);
    await this._init();
