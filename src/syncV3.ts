...
const XRegExp = (pattern: string, flags?: string): RegExp => {
  if (_XRegExpCache !== undefined) {
    return _XRegExpCache(pattern, flags);
  }
  try {
    // Try to get the already-cached CommonJS module
    const mod = (require as any)("xregexp");
    _XRegExpCache = (mod && mod.default) ? mod.default : mod;
    return (_XRegExpCache as any)(pattern, flags);
  } catch {
    // Fallback to native RegExp
    return new RegExp(pattern, flags);
  }
};

// Polyfill AggregateError for environments where it's not available
const AggregateError: new (errors: Iterable<any>, message?: string) => Error =
...
const getSyncPlanInplace = async (
  ...
  // Log all decisions
  for (const [key, entity] of Object.entries(mixedEntityMappings)) {
    log.debug(`[syncV3] decision key=${key} decision=${entity.decision} branch=${entity.decisionBranch}`);
  }
...
export const doActualSync = async (
  ...
  const totalCount = Object.keys(mixedEntityMappings).length;
  let uploadCount = 0;
  let downloadCount = 0;
  let deleteCount = 0;
  for (const entity of Object.values(mixedEntityMappings)) {
    const d = entity.decision;
    if (d === "local_is_created_then_push" || d === "local_is_modified_then_push" || d === "conflict_created_then_keep_local" || d === "conflict_modified_then_keep_local" || d === "folder_existed_local_then_also_create_remote") {
      uploadCount++;
    } else if (d === "remote_is_created_then_pull" || d === "remote_is_modified_then_pull" || d === "conflict_created_then_keep_remote" || d === "conflict_modified_then_keep_remote" || d === "folder_existed_remote_then_also_create_local") {
      downloadCount++;
    } else if (d === "local_is_deleted_thus_also_delete_remote" || d === "remote_is_deleted_thus_also_delete_local" || d === "folder_to_be_deleted_on_both" || d === "folder_to_be_deleted_on_local" || d === "folder_to_be_deleted_on_remote") {
      deleteCount++;
    }
  }
...
export async function syncer(
  ...
  try {
    ...
    step = 1;
    log.debug(`[syncV3] step: ${step} (init)`);
    await notifyFunc?.(triggerSource, step);
    ...
    step = 2;
    log.debug(`[syncV3] step: ${step} (check password)`);
    await notifyFunc?.(triggerSource, step);
    await ribboonFunc?.(triggerSource, step);
    await statusBarFunc?.(triggerSource, step, everythingOk);
    ...
    step = 7;
    log.debug(`[syncV3] step: ${step} (actual sync)`);
