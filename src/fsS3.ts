...
// Inline AggregateError to avoid ESM-only aggregate-error package in test environments
class AggregateError extends Error {
  errors: Error[];
  constructor(errors: Iterable<Error>) {
    const errs = [...errors];
    super(errs.map((e) => e.message).join("\n"));
...
////////////////////////////////////////////////////////////////////////////////
// requestTimeout helper (inlined to avoid ESM subpath import issues)
////////////////////////////////////////////////////////////////////////////////

const requestTimeout = (timeoutInMs = 0): Promise<never> => {
...
const getS3Client = (s3Config: S3Config) => {
  ...
  const region = s3Config.s3Region || "us-east-1";
...
export class FakeFsS3 extends FakeFs {
  ...
  constructor(s3Config: S3Config, vaultName: string, bypassCors: boolean) {
    super();
  ...
  async walk(): Promise<Entity[]> {
    ...
    log.debug(`[fsS3] walk: found ${res.length} entities`);
    return res;
  }
  ...
  async writeFile(
    ...
    log.debug(`[fsS3] writeFile: key=${key} size=${content.byteLength}`);
    const uploadFile = getRemoteWithPrefixPath(
      key,
      this.s3Config.remotePrefix ?? ""
    );
  ...
  async readFile(key: string): Promise<ArrayBuffer> {
    log.debug(`[fsS3] readFile: key=${key}`);
    if (key.endsWith("/")) {
  ...
  async checkConnect(callbackFunc?: any): Promise<boolean> {
    log.debug(`[fsS3] checkConnect: testing bucket=${this.s3Config.s3BucketName}`);
    try {
